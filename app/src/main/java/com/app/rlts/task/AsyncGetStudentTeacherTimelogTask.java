...
public class AsyncGetStudentTeacherTimelogTask extends AsyncTask<String, String, String> {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            url = new URL("http://192.168.0.107:3000/getStudentTeacherTimelog/web"); // change this
        } catch (Exception e) {
            e.printStackTrace();
            return e.getMessage();
        }
    ...
    protected void onPostExecute(String s) {
        //this.delegate.retrieveTimelog(studentTeacherTimelog);
        super.onPostExecute(s);

        if(studentTeacherTimelog.size() > 0){
            delegate.retrieveTimelog(studentTeacherTimelog);
        }else{
            studentTeacherTimelog.add(new Timelog(s, "test time", "test entryType", "test locationName", "test username"));
            delegate.retrieveTimelog(studentTeacherTimelog);
        }
    }
}
